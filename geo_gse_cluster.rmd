...
```{r}
info <- subset(gset@featureData@data, select=c("ID","Gene.symbol","Gene.title"))
plotex <- function(gene) {
    id <- which(info$Gene.symbol==gene)[1]
    qplot(1:14,ex[id,],geom = c("smooth","point"),xlab = "mouse 0-72h",ylab = gene)
}
# select genes need and normalize
selgeneid <- as.character(tT$ID[1:1000])
ex1000 <- ex[match(selgeneid,rownames(ex)),]
exnorm <- apply(ex1000,1,scale)  # matrix has been transversed
lbls <- rep(labels,each=2)
heatmap(exnorm,Rowv = NA,labRow=lbls)
cov1000 <- (t(exnorm) %*% exnorm)/13
